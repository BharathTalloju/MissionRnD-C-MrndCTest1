...
int merge_circularlists(struct node **head1, struct node **head2){
	...
	if (walker1 == NULL){
		while (walker2-> next != *head2){
			walker2 = walker2->next;
			len++;
			printf("\nCopied %d", walker2->data);
		}
		*head1 = *head2;
		return len;
	}
	else if (walker2 == NULL){
		while (walker1-> next != *head1){
			walker1 = walker1->next;
			len++;
			printf("\nCopied %d", walker1->data);
		}
		//*head1 = *head2;
		return len;
	}
	...
	len++;
	while (r_walker->next != *head1 && r_walker->next != *head2){
		printf("\nComparing %d and %d", walker1->data, walker2->data);
		...
		printf("\nCopied %d", r_walker->data);
		len++;
	}
	printf("R %d\n", r_walker->data);
	if (r_walker->next == *head1){
		printf("\nlist1 is complete");
		while (walker2 != *head2){
